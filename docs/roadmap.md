...
- [x] Telegram-бот управления (`src/bot.py`, stdlib long-polling): ссылка → отклик
...
- [x] Оркестратор (`src/apply.py`): скоринг → резюме → отклик; пауза, дневной лимит, анти-дубль
- [x] Лог откликов (`state/applications.jsonl`)
- [ ] **Кандидат:** авторизоваться `hh-applicant-tool authorize` (токен приложения)
- *Итог фазы:* присылаешь боту вакансию — он сам создаёт резюме, пишет письмо и откликается;
  ты управляешь паузой. Серый путь — см. [auto_apply.md](auto_apply.md).
...
| Риск | Митигировка |
|------|-------------|
| Бан аккаунта HH (серый путь) | команда `/pause`, дневной лимит, пауза перед действием, анти-дубль, **точечный** отклик только на присланную вакансию (не массово) |
