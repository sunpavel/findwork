...
COVER_LETTER_SPEC = """СТРУКТУРА И ОФОРМЛЕНИЕ ПИСЬМА — соблюдай буквально:
Письмо — РОВНО 5 коротких АБЗАЦЕВ ПРОЗОЙ, разделённых ПУСТОЙ СТРОКОЙ. БЕЗ списков и маркеров.
Объём 150–250 слов. Внутри абзаца переносов строк нет. Логика абзацев:

Здравствуйте!

<Абзац 1 — почему вакансия логично связана с опытом кандидата; понимание задач/боли бизнеса
ИМЕННО этой компании. Конкретику бери из текста вакансии, название компании вплети естественно.>

<Абзац 2 — самый релевантный опыт под эту вакансию: только то, что совпадает с задачами
работодателя, с названиями компаний из мастера. Не весь опыт подряд.>

<Абзац 3 — 2–4 конкретных достижения В ЦИФРАХ из мастера (мин. 3 проверяемые метрики:
выручка/прибыль, рост в %/разах, доли/Топ, CAC/LTV/ROMI/CPL, бюджеты, размеры команд/сети).>

<Абзац 4 — почему кандидат будет полезен ИМЕННО в этой роли: связь его опыта с задачами вакансии.>

Буду рад обсудить, как мой опыт может помочь в достижении целей вашей компании.
...
ПРАВИЛА ОФОРМЛЕНИЯ:
- РОВНО 5 смысловых абзацев + приветствие + подпись. Между абзацами — одна пустая строка.
- БЕЗ списков/буллитов: задачи вакансии и соответствие им показывай прозой внутри абзацев 2 и 4.
- Подпись — отдельным блоком в конце, каждый контакт на своей строке, ровно эти контакты (не менять).
- Тире в тексте только короткое «-» (длинные «—» не использовать).

ЭТАЛОН СТИЛЯ, ТОНА И ПЛОТНОСТИ ЦИФР. ВАЖНО: ниже — ВЫМЫШЛЕННЫЙ кандидат из ДРУГОЙ отрасли. Это
образец только СТРУКТУРЫ (5 абзацев прозой), ТОНА и ПЛОТНОСТИ ЦИФР. Компании, отрасль, цифры и
формулировки НЕ копируй — все факты бери СТРОГО из мастер-резюме под конкретную вакансию (иначе
выдумка). ОСОБЕННО не копируй зацепку первого абзаца — в реальном письме она про специфику ИМЕННО
твоей вакансии. Подпись — каноническая (контакты кандидата), её оставляй как есть:
<<<
Здравствуйте!

Судя по описанию роли, ключевая задача — перезапустить онлайн-канал сети и связать маркетинг с
экономикой продаж, а не просто «крутить рекламу». Именно такие задачи я закрывал в e-grocery.

В сети «ФрешМаркет» я отвечал за маркетинг и коммерцию направления с оборотом 2,4 млрд ₽ и
выстраивал онлайн-канал как полноценный P&L, а не витрину.

Там я вывел выручку онлайн-канала с 320 до 540 млн ₽ за год, поднял средний чек на 18% и снизил
CAC на 25% при росте базы активных клиентов в 2 раза за счёт сквозной аналитики и работы с LTV.

Для вашей роли это означает, что я приду не «настраивать каналы», а отвечать за рост выручки и
unit-экономику: тот же подход — аналитика, retention, связка маркетинга, продукта и продаж.
...
def _tailor_template(vacancy: dict, matched_skills: list[str] | None) -> TailorResult:
    ...
    cover = format_cover_letter(
        ...
        "— масштабирование: федеральная сеть с 3 до 15 комплексов, +40% средний чек, +50% поток;\n"
