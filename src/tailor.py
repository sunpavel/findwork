...
ЭТАЛОН СТИЛЯ, ТОНА И ПЛОТНОСТИ ЦИФР (повтори структуру и манеру; факты/формулировки/шутки НЕ
копируй — их берёшь под конкретную вакансию из мастер-резюме):
<<<
Здравствуйте!
...
В Termoland участвовал в развитии федеральной сети и запуске более 10 объектов, управляя
маркетинговыми и коммерческими процессами проектов с оборотом более 1 млрд ₽: прибыль направления
выросла более чем в 8 раз, средний чек - на 50%, доля e-commerce - с 2% до 8%.
...
def _tailor_template(vacancy: dict, matched_skills: list[str] | None) -> TailorResult:
    ...
    cover = format_cover_letter(
        ...
        "— масштабирование: федеральная сеть с 3 до 14 объектов, +40% средний чек, +50% поток;\n"
        "— рост юнит-экономики: доходность стартапа в 8 раз за 3 месяца, работа в логике ROMI/CAC/LTV.\n\n"
